...
const Login = () => {
  ...
  const [theme, setTheme] = useState('deadpool');
  const [isLoading, setIsLoading] = useState(false);
  const navigate = useNavigate();
  const { toast } = useToast();

  // Apply themes based on selection
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen flex justify-center items-center p-4 bg-background">
      <div className="w-full max-w-md">
        <SlideUp className="glass-panel rounded-xl overflow-hidden">
          <form onSubmit={handleEmailLogin} className="p-8 space-y-5">
            <FadeIn className="text-center mb-6">
              <motion.div 
                ...
                whileTap={{ scale: 0.9 }}
              >
                <Calendar className="h-12 w-12 text-primary" strokeWidth={1.5} />
              </motion.div>
              <p className="text-2xl font-bold">
                Welcome, <span className="block text-sm text-muted-foreground mt-1">sign in to continue</span>
              </p>
            </FadeIn>

            <div className="theme-toggle mb-6">
            ...
            <Button 
              type="button" 
              variant="outline"
              className="w-full h-11 justify-start px-4 font-normal"
